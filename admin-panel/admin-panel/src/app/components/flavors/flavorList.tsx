...
import { Search, MoreHorizontal, Trash2, Edit3, Pipette } from "lucide-react";
  ...
  return (
    <div className="bg-card rounded-3xl border border-border-theme shadow-sm overflow-hidden">
      ...
      {loading ? (
        ...
        <div className="p-20 text-center text-slate-400 italic">No flavors found.</div>
      ) : (
                ...
                <tr key={f._id} className="hover:bg-hover-theme transition-all duration-300 group border-b border-border-theme/50">
                  <td className={adminTableBodyCellClass}>
                    <div className="h-10 w-14 rounded-xl border border-slate-100 bg-slate-50 overflow-hidden shrink-0">
                      {f.image ? <img src={f.image} className="h-full w-full object-cover" alt="" /> : <Pipette className="h-full w-full p-3 text-slate-200" />}
                    </div>
                  </td>
                  <td className={adminTableBodyCellClass}>
      ...
      <div className="p-6 border-t border-slate-100 bg-white flex items-center justify-between">
        <div className="text-xs font-bold text-slate-400 uppercase tracking-widest font-sans">
          Showing {(currentPage - 1) * 10 + Math.min(1, flavors.length)}-{Math.min(currentPage * 10, total)} of {total}
        </div>
